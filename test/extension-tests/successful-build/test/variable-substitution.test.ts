...
suite('[Variable Substitution]', async () => {
  ...
  test('Check substitution for "workspaceRoot"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[workspaceRoot] configure failed');
  ...
  test('Check substitution for "workspaceFolder"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[workspaceFolder] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "buildType"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[buildType] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "buildKit"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[buildKit] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "workspaceRootFolderName"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[workspaceRootFolderName] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "workspaceFolderBasename"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[workspaceFolderBasename] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "generator"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[generator] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for "userHome"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[userHome] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
  ...
  test('Check substitution for variant names', async () => {
    ...
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[variant] configure failed');
  ...
  test('Check substitution within "cmake.installPrefix"', async () => {
    ...
    // Configure
    expect(await cmt.configure([], ConfigureType.Normal, ConfigureTrigger.runTests)).to.be.eq(0, '[cmakeInstallPrefix] configure failed');
    expect(testEnv.projectFolder.buildDirectory.isCMakeCachePresent).to.eql(true, 'expected cache not present');
    const cache = await CMakeCache.fromPath(await cmt.cachePath);
