...
export function makeCodeModelDriverTestsuite(driverName: string, driver_generator: (cmake: CMakeExecutable, config: ConfigurationReader, kit: Kit | null, workspaceFolder: string, preconditionHandler: CMakePreconditionProblemSolver, preferredGenerators: CMakeGenerator[]) => Promise<CMakeDriver>) {
    // Skip tests for the Server driver on Windows, as it is not supported.
    if (!(driverName === "Server" && process.platform === 'win32')) {
        suite(`CMake CodeModel ${driverName} Driver tests`, () => {
            ...
            const sourceOutsideOfWorkspace = getTestRootFilePath(
                "test/unit-tests/driver/workspace/source_outside_of_workspace/workspace"
            );
            ...
            setup(async function (this: Mocha.Context, done) {
                ...
                if (
                    !cleanupBuildDir(
                        path.join(sourceOutsideOfWorkspace, "build")
                    )
                ) {
                    done(
                        "Source-outside-of-workspace project build folder still exists"
                    );
                }
            ...
            async function generateCodeModelForConfiguredDriver(
                ...
                expect(
                    (await driver.configure(ConfigureTrigger.runTests, args))
                        .result
                ).to.be.eq(0);
                return code_model;
            }
            ...
            test("Test generation of code model with multi configuration like VS", async () => {
                ...
                const codemodel_data =
                    await generateCodeModelForConfiguredDriver();
            ...
            test("Test project information", async () => {
                ...
                expect(
                    path.normalize(project.sourceDirectory).toLowerCase()
                ).to.eq(
                    path
                        .normalize(path.join(root, "test_project"))
                        .toLowerCase()
                );
            }).timeout(90000);
            ...
            test("Test executable target information", async () => {
                ...
                const target =
                    codemodel_data!.configurations[0].projects[0].targets.find(
                        (t) =>
                            t.type === "EXECUTABLE" &&
                            t.name === "TestBuildProcess"
                    );
                ...
                // used by tree view to make paths relative
                expect(
                    path.normalize(target!.sourceDirectory!).toLowerCase()
                ).to.eq(
                    path
                        .normalize(path.join(root, "test_project"))
                        .toLowerCase()
                ...
                const compile_information = target!.fileGroups!.find(
                    (t: any) => !!t.language
                );
            ...
            test("Test first static library target directory", async () => {
                ...
                expect(
                    ...
                    path
                        .normalize(
                            path.join(root, "test_project", "static_lib_dummy")
                        )
                        .toLowerCase()
                );
                ...
                if (process.platform === "win32") {
                    expect(
                        compile_information!.compileCommandFragments
                            ?.map((str) => str.trim())
                            .join(" ")
                    ).to.eq(
            ...
            test("Test first shared library target directory", async () => {
                ...
                expect(
                    ...
                    path
                        .normalize(
                            path.join(root, "test_project", "shared_lib_dummy")
                        )
                        .toLowerCase()
                );
                ...
                expect(target!.fileGroups![0].defines).contains(
                    "TEST_CMAKE_DEFINE"
                );
                expect(target!.fileGroups![0].isGenerated).to.be.false;
                expect(
                    path
                        .normalize(target!.fileGroups![0].includePath![0].path)
                        .toLowerCase()
                ).to.eq(
                    path
                        .normalize(
                ...
                if (process.platform === "win32") {
                    expect(
                        target!
                            .fileGroups![0].compileCommandFragments?.map(
                            (str) => str.trim()
                        )
                            .join(" ")
                    ).to.eq("/DWIN32 /D_WINDOWS /W3 /MDd /Zi /Ob0 /Od /RTC1");
                }
            }).timeout(90000);
            ...
            test("Test cache access", async () => {
                ...
                const target =
                    codemodel_data!.configurations[0].projects[0].targets.find(
                        (t: any) =>
                            t.type === "UTILITY" && t.name === "runTestTarget"
                    );
                expect(target).to.be.not.undefined;
            ...
            test("Test source files outside of workspace root", async () => {
                ...
                const codemodel_data =
                    await generateCodeModelForConfiguredDriver(
                        [],
                        sourceOutsideOfWorkspace
                    );
                expect(codemodel_data).to.be.not.null;

                for (const [target_name, target_subdir, sourcefile_name] of [
                    ["root_target", "", "../main.cpp"],
                    ["subdir_target", "subdir", "../../main.cpp"]
                ] as const) {
                    const target =
                        codemodel_data!.configurations[0].projects[0].targets.find(
                            (t: any) =>
                    ...
                    expect(sources).to.include(
                        path.normalize(sourcefile_name).toLowerCase()
                    );
                }
            }).timeout(90000);
        });
