...
export function makeDriverTestsuite(driverName: string, driver_generator: (cmake: CMakeExecutable, config: ConfigurationReader, kit: Kit | null, workspaceFolder: string, preconditionHandler: CMakePreconditionProblemSolver, preferredGenerators: CMakeGenerator[]) => Promise<CMakeDriver>) {
    ...
    // Skip the tests for the Server driver on Windows, as it is not supported.
    if (!(driverName === "Server" && process.platform === 'win32')) {
        suite(`CMake ${driverName} driver tests`, () => {
            const cmakePath: string = process.env.CMAKE_EXECUTABLE
                ? process.env.CMAKE_EXECUTABLE
                : "cmake";
            const defaultWorkspaceFolder = getTestRootFilePath(
                "test/unit-tests/driver/workspace/test_project"
            );
            const emptyWorkspaceFolder = getTestRootFilePath(
                "test/unit-tests/driver/workspace/empty_project"
            );
            const badCommandWorkspaceFolder = getTestRootFilePath(
                "test/unit-tests/driver/workspace/bad_command"
            );
            ...
            setup(async function (this: Mocha.Context, done) {
                ...
                if (
                    !cleanupBuildDir(path.join(defaultWorkspaceFolder, "build"))
                ) {
                    done("Default build folder still exists");
                }

                if (
                    !cleanupBuildDir(path.join(emptyWorkspaceFolder, "build"))
                ) {
                ...
                if (
                    !cleanupBuildDir(
                        path.join(badCommandWorkspaceFolder, "build")
                    )
                ) {
            ...
            test(`All target for ${ninjaKitDefault.name}`, async () => {
                ...
                const executable = await getCMakeExecutableInformation(
                    cmakePath
                );

                driver = await driver_generator(
                    executable,
                    config,
                    ninjaKitDefault,
            ...
            test("Check binary dir", async () => {
                ...
                expect(driver.binaryDir).to.endsWith(
                    "test/unit-tests/driver/workspace/test_project/build"
                );
            }).timeout(60000);
            ...
            test("Build", async () => {
                ...
                const targetInTopLevelBuildDir = driver.executableTargets.find(
                    (t) => t.name === "TestBuildProcess"
                );
                expect(targetInTopLevelBuildDir).to.not.undefined;
                expect(fs.existsSync(targetInTopLevelBuildDir!.path)).to.be
                    .true;

                const targetInRuntimeOutputDir = driver.executableTargets.find(
                    (t) => t.name === "TestBuildProcessOtherOutputDir"
                );
                expect(targetInRuntimeOutputDir).to.not.undefined;
                expect(fs.existsSync(targetInRuntimeOutputDir!.path)).to.be
                    .true;
            }).timeout(90000);
            ...
            test("Configure fails on invalid preferred generator", async () => {
                ...
                const kit = {
                    name: "GCC",
                    preferredGenerator: { name: "invalid Name" }
                } as Kit;

                try {
                    await driver_generator(
                        executable,
                        config,
                        kit,
                        defaultWorkspaceFolder,
                    ...
                    expect(
                        false,
                        "configure did not detect the invalid generator"
                    ).to.be.true;
                } catch (e) {
                    if (!(e instanceof NoGeneratorError)) {
                        expect(
                            false,
                            `configure threw the wrong Error type: ${typeof e}`
                        ).to.be.true;
                    }
                }
            }).timeout(60000);
            ...
            test("Test compiler name reporting for telemetry", async () => {
                ...
                let compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/aarch64_clang++"
                );
                expect(compilerInfo.name).to.be.eq("clang++-aarch64");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/clang_eabi"
                );
                expect(compilerInfo.name).to.be.eq("clang-eabi");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/cl.exe"
                );
                expect(compilerInfo.name).to.be.eq("cl");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/arm-xlc++"
                );
                expect(compilerInfo.name).to.be.eq("xlc++-arm");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/other-prefix-gcc"
                );
                expect(compilerInfo.name).to.be.eq("gcc");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/cc-other-suffix"
                );
                expect(compilerInfo.name).to.be.eq("cc");
                compilerInfo = await driver.getCompilerVersion(
                    "drive/folder/path/unknown-arm64-compiler"
                );
            ...
            test("Set kit without a preferred generator", async () => {
                ...
                expect(
                    (await driver.configure(ConfigureTrigger.runTests, []))
                        .result
                ).to.be.eq(0);
                const kit2 =
                    driver.cmakeCacheEntries?.get("CMAKE_GENERATOR")!.value;
            ...
            test("Try build on empty dir", async () => {
                ...
                let called = false;
                const checkPreconditionHelper = async (
                    e: CMakePreconditionProblems
                ) => {
                    expect(e).to.be.eq(
                        CMakePreconditionProblems.MissingCMakeListsFile
                    );
                    called = true;
                    return false;
                };
            ...
            test("No parallel configuration", async () => {
                ...
                const checkPreconditionHelper = async (
                    ...
                    expect(e).to.be.eq(
                        CMakePreconditionProblems.ConfigureIsAlreadyRunning
                    );
                    called = true;
                    return false;
                };
            ...
            test("No build parallel to configure", async () => {
                ...
                expect(await configure).to.be.deep.equal({
                    result: 0,
                    resultType: ConfigureResultType.NormalOperation
                });
            ...
            test("Test pre-configured workspace", async () => {
                ...
                expect(driver.generatorName).to.be.eq(
                    kitBaseline.preferredGenerator!.name
                );
                expect(
                    driver.cmakeCacheEntries.get("CMAKE_GENERATOR")!.value
                ).to.be.eq(kitBaseline.preferredGenerator!.name);
            }).timeout(60000);
            ...
            test("Test Visual Studio kit with wrong all target name", async () => {
                ...
                await driver.cleanConfigure(ConfigureTrigger.runTests, []);
                expect(await driver.build(["all"])).to.be.eq(
                    0,
                    "Automatic correction of all target failed"
                );
            }).timeout(90000);
            ...
            test("Test Ninja kit with wrong all target name", async () => {
                ...
                await driver.cleanConfigure(ConfigureTrigger.runTests, []);
                expect(await driver.build(["ALL_BUILD"])).to.be.eq(
                    0,
                    "Automatic correction of ALL_BUILD target failed"
                );
            }).timeout(90000);
            ...
            test("Test extra arguments on configure", async () => {
                ...
                await driver.configure(ConfigureTrigger.runTests, [
                    "-DEXTRA_ARGS_TEST=Hallo"
                ]);
                expect(
                    driver.cmakeCacheEntries.get("extraArgsEnvironment")?.value
                ).to.be.eq("Hallo");
            }).timeout(90000);
