...
// TODO: Likely needs fixing since we're creating PRs in different locations. 
interface Settings {
  orgUrl: string;
  project: string;
  repo: string;
...
// For local testing, set the SYSTEM_ACCESSTOKEN environment variable to a PAT with write access
...
class Session {
  ...
  private async getRepos(): Promise<GitRepository[]> {
    ...
    return await this.git.getRepositories(this.settings.project);
  }
  ...
  private prepareSourceBranch(): void {
    ...
    try {
      ...
      // add all modified i18n files
      // TODO: This is hardcoded to the i18n root directory, will need to be modified.
