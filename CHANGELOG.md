...
Bug Fixes:
- Fix CMake Tools showing unsolicited quick picks on workspace open (`cmake.configureOnOpen`) — kit/configure-preset selection and the "Select CMakeLists.txt" prompt — which could cancel a quick pick you had just opened yourself (e.g. the command palette), since VS Code's quick input is a single shared widget. These startup prompts are now surfaced as non-modal notifications you can act on when ready; explicit configure/build commands still prompt directly, and the status bar continues to show the "No Kit Selected"/"No Configure Preset Selected" state. [#5000](https://github.com/microsoft/vscode-cmake-tools/issues/5000)
