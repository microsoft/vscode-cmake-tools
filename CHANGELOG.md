...
- Do not auto-pop a quick pick for kit / configure-preset / CMakeLists.txt selection during workspace open. The startup path now shows a non-modal information notification with a "Select" button when CMake Tools needs a choice; the quick pick only opens when the user clicks the button or invokes the manual command. This stops the startup prompts from cancelling any quick pick the user or another extension already has on screen during activation. [#5000](https://github.com/microsoft/vscode-cmake-tools/issues/5000)
