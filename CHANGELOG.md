...
- Replace the datalist/input control with a select control for enum-valued cache entries in the CMake Cache Editor, so allowed values are chosen from a proper dropdown instead of a free-text field. [#4973](https://github.com/microsoft/vscode-cmake-tools/pull/4973)
...
- Fix a single test's "Run" action in a multi-project workspace running all tests for that project instead of just the selected test, caused by test information from the most recently refreshed project overwriting the shared state used to compute the test filter. [#4911](https://github.com/microsoft/vscode-cmake-tools/issues/4911)
