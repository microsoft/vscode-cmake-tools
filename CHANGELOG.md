...
- Pin the VS Code version used by the test harness (`@vscode/test-electron`) to 1.130.0 so macOS (Apple Silicon) CI can launch VS Code; the latest stable (1.131.0) dropped the legacy Electron symlink that the harness expects. Override with the `VSCODE_TEST_VERSION` environment variable. [#5011](https://github.com/microsoft/vscode-cmake-tools/issues/5011)
- Make Windows E2E build-directory cleanup resilient to transient file locks so the test suite no longer intermittently fails with `EPERM`/`EBUSY`/`ENOTEMPTY` when a lingering MSVC helper process (e.g. mspdbsrv.exe / vctip.exe) or antivirus scanner briefly holds a handle: the recursive delete now rides out such locks with a much larger retry budget and, as a last resort, tolerates a transient lock during cleanup instead of failing the test.
...
Bug Fixes:
- Fix kit selection targeting the wrong folder in a multi-root workspace when a non-active folder triggers configure-on-open. The "Select a Kit for &lt;name&gt;" prompt now shows and applies the kit to the folder that actually needs it, instead of the active folder, so the prompt no longer reappears on every reload. [#5011](https://github.com/microsoft/vscode-cmake-tools/issues/5011)
