...
Improvements:
- Improve CMake syntax highlighting: add command-scoped keyword rules for `if()`, `list()`, `string()`, `message()`, `project()`, `set()`, `option()`, `install()`, `foreach()`, and `while()` so that sub-command keywords (e.g. `APPEND`, `TOLOWER`, `STATUS`, `FATAL_ERROR`) and operators (e.g. `AND`, `OR`, `STREQUAL`) are highlighted consistently and only inside their respective commands. Remove obsolete deprecated keyword entries (`SOURCE_FILES`, `BUILD_NAME`, etc.) that caused false-positive highlighting on user-defined variable names. [#4709](https://github.com/microsoft/vscode-cmake-tools/issues/4709) [#4508](https://github.com/microsoft/vscode-cmake-tools/issues/4508) [#4613](https://github.com/microsoft/vscode-cmake-tools/issues/4613)
