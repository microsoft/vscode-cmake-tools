...
- Add a localization verification check (run in CI on every pull request) that keeps the translated strings under `i18n/` healthy: it prevents human-reviewed translations recorded in `jobs/loc/translation-fixes.json` from being silently reverted by the automatic localization import, and flags any translation that drops or mangles a message placeholder (`{0}`) or expansion variable (`${...}`). A `translations-restore` task restores reverted translations automatically.
