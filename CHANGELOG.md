...
Improvements:
- The colorized CMake Build terminal now honors `cmake.revealLogOnAutomaticTrigger`: it opens promptly for builds you start yourself but stays quiet for automatic and programmatic builds (e.g. configure-on-open and builds invoked through the CMake Tools API by Copilot via the C/C++ DevTools companion), and it now opens immediately when you start a build instead of leaving you on configuration progress while a configure runs first. Failures are always shown. [#478](https://github.com/microsoft/vscode-cmake-tools/issues/478)
