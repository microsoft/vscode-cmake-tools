...
- Keep the translated strings under `i18n/` healthy across automatic localization updates. Human-reviewed translation fixes are recorded in `jobs/loc/translation-fixes.json` and restored automatically before the localization pull request is opened, so they are no longer silently reverted. A Localization Readiness check surfaces the result directly on that pull request — a comment and a `localization: not ready` label when an import reverts a reviewed fix or drops/mangles a message placeholder (`{0}`) or expansion variable (`${...}`), so it is obvious before merging — and the ledger is also verified on ordinary pull requests to catch manual regressions.
