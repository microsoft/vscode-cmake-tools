...
Bug Fixes:
- Fix the Test Explorer's "Run Tests" and "Run Tests with Coverage" profiles always dispatching through whichever CMake project happened to initialize the (shared) test controller first, in multi-root workspaces with more than one CMake project. Running a test belonging to any other project would fail with "Could not determine the test program for test ...". The Run and Coverage profiles now resolve the owning project from the clicked test's URI, the same way the Debug profile already did. [@danniesim](https://github.com/danniesim)
