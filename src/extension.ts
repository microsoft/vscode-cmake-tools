...
import {CMakeToolsFolder, CMakeToolsFolderController} from '@cmt/folders';
import {effectiveKitEnvironment, findCLCompilerPath, Kit, USER_KITS_FILEPATH} from '@cmt/kit';
import {KitsController} from '@cmt/kitsController';
import * as logging from '@cmt/logging';
import {fs} from '@cmt/pr';
import {FireLate, FireNow} from '@cmt/prop';
import rollbar from '@cmt/rollbar';
import {StatusBar} from '@cmt/status';
import * as telemetry from '@cmt/telemetry';
import {ProjectOutlineProvider, SourceFileNode, TargetNode} from '@cmt/tree';
import * as util from '@cmt/util';
import {DummyDisposable, ProgressHandle, reportProgress} from '@cmt/util';
...
class ExtensionManager implements vscode.Disposable {
    ...
    cleanConfigure(folder?: vscode.WorkspaceFolder) {
        return this.mapCMakeToolsFolder(cmt => cmt.cleanConfigure(), folder, true);
    }

    cleanConfigureAll() {
        return this.mapCMakeToolsAll(cmt => cmt.cleanConfigure(), true);
    }

    configure(folder?: vscode.WorkspaceFolder) {
        return this.mapCMakeToolsFolder(cmt => cmt.configure(), folder, true);
    }

    configureAll() {
        return this.mapCMakeToolsAll(cmt => cmt.configure(), true);
    }

    build(folder?: vscode.WorkspaceFolder, name?: string) {
        return this.mapCMakeToolsFolder(cmt => cmt.build(name), folder, true);
    }

    buildAll(name?: string) {
        return this.mapCMakeToolsAll(cmt => cmt.build(name), true);
    }
