 ...
 */
export function getTargets(cmakeTools: CMakeTools): TargetInformation[] {
  return cmakeTools.targets.map(target => ({cmakeTools, target}));
}
...
export class TargetProvider implements vscode.Disposable {
  ...
  registerCMakeTools(cmt: CMakeTools) {
