...
export class CMakeTools implements vscode.Disposable, api.CMakeToolsAPI {
  ...
  async debugTarget(name?: string): Promise<vscode.DebugSession|null> {
    ...
    if (this.buildPreset?.environment) {
      ...
      debug_config.environment = debug_config.environment ? debug_config.environment.concat(build_preset_environment.debugConfigEnvironment) : [];
    }
  ...
  async launchTarget(name?: string) {
    ...
    if (drv && this.buildPreset?.environment) {
      const build_preset_environment = await drv.getConfigureEnvironment();
      user_config.environment = user_config.environment ? user_config.environment.concat(build_preset_environment.debugConfigEnvironment) : [];
    }
