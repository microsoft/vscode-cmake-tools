...
export class CMakeFileApiDriver extends codemodel.CodeModelDriver {
  ...
  async doInit(autoConfigInternal: boolean) {
    ...
    if (cacheExists && this.generator?.name === await this.getGeneratorFromCache(this.cachePath)) {
      ...
      if (!code_model_exist) {
        await this.doConfigure([], autoConfigInternal, undefined);
      }
    } else {
      if (cacheExists) {
  ...
  async doConfigure(args_: string[], autoConfigInternal: boolean = false, outputConsumer?: proc.OutputConsumer): Promise<number> {
