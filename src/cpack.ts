...
import { packageArgs, PackagePreset } from './preset';
...
const magicKey = 'cpack.magic.key';
// Used as magic value
let sessionNum= 0;

// Placeholder in the cpack explorer when cpack preset is not selected
const testPresetRequired = '_package_preset_required_';

type ConfigPackageStatus = ('failed' | 'notrun' | 'finished');
...
export class CPackDriver implements vscode.Disposable {
    ...
    private async getCPackArgs(driver: CMakeDriver, packagePreset?: PackagePreset): Promise<string[] | undefined> {
        ...
        const defaultArgs = [];
        for (const value of this.ws.config.cpackDefaultArgs) {
        ...
        // add cpack variables, -D var=val...

        cpackArgs = cpackArgs.concat(defaultArgs, args);
        return cpackArgs;
    }
