...
| `cmake.modifyLists.addNewSourceFiles` | Add source files to CMake lists when they are created. `"no"` disables, `"yes"` applies automatically, `"ask"` shows a confirmation before applying. | `"ask"` | no |
| `cmake.modifyLists.removeDeletedSourceFiles` | Remove source files from CMake lists when they are deleted. `"no"` disables, `"yes"` applies automatically, `"ask"` shows a confirmation before applying. | `"ask"` | no |
