...
| `cmake.environment` | An object containing `key:value` pairs of environment variables, which will be available when configuring, building, debug, running or testing with CTest. | `null` (no environment variables) | yes |
