...
| `cmake.configureOnEdit` | Automatically configure CMake project directories when `cmake.sourceDirectory` path in the settings is updated or `CMakeLists.txt` content is saved. | `true` | no |
